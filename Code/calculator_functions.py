    ...
    for date in date_list:
        ...
        if os.path.exists(day_path):
            ...
            for pat_string in os.listdir(day_path):
                ...
                # cache listing once
                files_in_pat = set(os.listdir(pat_path))

                for file in files_in_pat:
                    # our MS naming: "MS%<stem>.dcm"
                    if file.startswith('MS%') and file.lower().endswith('.dcm'):
                        # everything after the first '%' is the common stem
                        stem = file.split('%', 1)[1]  # e.g. 'limbus_...P0001C.dcm'

                        # expected matching AS filename
                        as_name = f'AS%{stem}'

                        # expected scores filename (no extension in stem)
                        stem_noext, _ = os.path.splitext(stem)
                        scores_name = f'scores_{stem_noext}.npy'

                        if as_name in files_in_pat:
                            if not os.path.exists(os.path.join(pat_path, scores_name)):
                                to_do.append((
                                    os.path.join(pat_path, file),     # MS full path
                                    os.path.join(pat_path, as_name)   # AS full path
                                ))
    if len(to_do) != 0:
